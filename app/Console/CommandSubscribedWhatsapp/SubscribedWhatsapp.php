...
class SubscribedWhatsapp extends Command
{
      ...
      foreach ($notifications  as $notification ) {
        ...
        if(  $notification->queue_process_status  == 'running'   ){
          ...
          //  continue;
            ...
            $log =  "\nIncoming Webhook Job Queue Id: {$notification->incoming_webhook_job_queue_id} | Status: Already Processed ";                  
            echo $log;    
            continue;
    ...
    public function sendRegistrationWhatsapp($notification)
    {
      ...
      $mobile_country_codes = DB::table('mobile_country_list')
      ->where('country_code', $countryCode)
      ->get();

      $country_code = [];

      if($mobile_country_codes ){
        foreach($mobile_country_codes  as $value ){

           $country_code[] =  $value->mobile_country_code_id;

        }
        $countryCodesString = implode(',', $country_code);
      }   
      ...
      if($unsubscribed_keyword){
        foreach($unsubscribed_keyword as $unsubscribe){
           ...
           if((strtolower($text_msg) == strtolower($unsubscribe)) ){
              ...
              $query = "
                          UPDATE ". dbPrefix() ."event_program_registration
                          SET whatsapp_optin = 'N'
                          WHERE mobile_country_code_id IN ($countryCodesString)
                          AND mobile_no = ?
                      
                      ";

              DB::statement($query, [$mobileNumber]);
