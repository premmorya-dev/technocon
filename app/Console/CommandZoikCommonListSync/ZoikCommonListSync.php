...
namespace App\Console\CommandZoikCommonListSync;
...
class ZoikCommonListSync extends Command
{
  /**
   ...
   * @var string
   */
  protected $signature = 'app:zoikmail-common-list-sync {limit?}';
  ...
  public function handle()
  {
    ...
    $notifications = DB::table('notification_job_queue')
      ...
      ->where('notification_job_queue_type', 'zoikmail-common-list-sync')
    ...
    if (isset($registration_data['zoik_app_id']) && isset($registration_data['zoik_app_common_list_uid']) && $registration_data['zoik_app_common_list_uid']) {
      $zoik_app_setting =   DB::table('zoik_app_setting')
       ...
       try{
        if ($zoik_app_setting) {
          ...
          if ($registration_data['zoik_app_common_list_field_mapping']) {
            ...
            if ($common_list_param->itemAt('status') == 'success') {
              ...
              $search_common_response = $endpointListSubscribers->emailSearch($registration_data['zoik_app_common_list_uid'], $registration_data['registered_email']);
              ...
              if ($search_common_response) {
                ...
                if ($search_common_param->itemAt('status') ==  'success') {
                  ...
                  $update_response = $endpointListSubscribers->update($registration_data['zoik_app_common_list_uid'], $search_common_param->itemAt('data')['subscriber_uid'], $sync_data);
                  ...
                  if ($update_response) {
                    ...
                    if ($update_param->itemAt('data')) {
                      ...
                      $log = "\nRegistration Id: {$notification->registration_id} | Notification Type: {$notification->notification_job_queue_type} | Status: success | Msg: Common list subscriber UID updated | List UID: {$registration_data['zoik_app_common_list_uid']} | Subscriber UID: {$search_common_param->itemAt('data')['subscriber_uid']}";
                  ...
                  if ($create_common_list_response) {
                    ...
                    $log = "\nRegistration Id: {$notification->registration_id} | Notification Type: {$notification->notification_job_queue_type} | Status: success | Msg: Common list subscriber UID created | List UID: {$registration_data['zoik_app_common_list_uid']} | Subscriber UID: {$create_common_list_param->itemAt('data')['record']['subscriber_uid']}";
        ...
        $log = "\nRegistration Id: {$notification->registration_id} | Notification Type: {$notification->notification_job_queue_type} | Status: failed | Msg:" . $e->getMessage();
