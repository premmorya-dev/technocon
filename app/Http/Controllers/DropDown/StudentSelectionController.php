...
use App\Models\DropDown\StudentSelectionModel;
...
class StudentSelectionController extends Controller
{
    ...
    {
        $query = StudentSelectionModel::query();

        // dd($query);

        // Apply filters
        if ($request->filled('filter_selection_status')) {
            $query->where('selection_status', 'like','%'. $request->input('filter_selection_status') .'%');
        }
        if ($request->filled('filter_bootstrap_class')) {
            $query->where('bootstrap_class', 'like', '%' . $request->input('filter_bootstrap_class') . '%');
        }
        if ($request->filled('filter_selection_status_backend')) {
            $query->where('selection_status_backend', 'like', '%' . $request->input('filter_selection_status_backend') . '%');
        }
        if ($request->filled('filter_selection_status_description')) {
            $query->where('selection_status_description', 'like', '%' . $request->input('filter_selection_status_description') . '%');
        }


        $sd_selections = $query->sortable()->paginate(setting('pagination_limit'));
        ...
        return view('pages/dropdown.student_selection.list', compact('sd_selections'));
